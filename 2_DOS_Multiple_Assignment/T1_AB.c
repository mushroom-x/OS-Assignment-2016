...
/* SEMAPHORE */
semaphore FreeBufMutex = {1,NULL};           
semaphore FreeBufNum = {MAX_BUF_NUM,NULL};     
semaphore TcbMutex = {1,NULL};         
semaphore TcbEmpty = {MAX_TCB_NUM,NULL};         
semaphore TcbFull = {0,NULL}; 
...
int main()
{
	...
	/*ThreadCreateAt(0,"main",(codeptr)T0FinishedCheck,STACK_SIZE);*/
    
    tcbs[0].state = RUNNING;
    current = 0;

    ThreadCreate("T1",(codeptr)T1PrintA,STACK_SIZE);
	ThreadCreate("T2",(codeptr)T2PrintB,STACK_SIZE);
	
    /*TODO there is a bug ,i don't know why*/
    /* ThreadCreateAt(0,"T0",(codeptr)T0FinishedCheck,STACK_SIZE); */
...
/**
 * [T0FinishedCheck #0 ]
 * Monitor all the thread except for thread #0.
 * To check if they are all end.
 * if so end the entire system. 
 */

/*
void T0FinishedCheck()
{
    printf("T0 is Running\n");
    while(!ThreadFinished())
    {
        printf("_");
        my_delay(5);
...
/**
 * [T1PrintA #1]
 * Repeat output character "a"
 * In the end ,destory itself
 */
void T1PrintA()
{
	int i;
